...
def solve_knapsack_gurobi(n, capacity, values, weights):
...
def evaluate_instance(filename):
    ...
    opt_value, gurobi_time, gurobi_sol = solve_knapsack_gurobi(n, capacity, values, weights)
    results['gurobi'] = {'value': opt_value, 'time': gurobi_time}
...
if __name__ == "__main__":
    results = evaluate_instance("instance1.txt")
    
    # Print results
    print("\nResults:")
    print("Gurobi Solution:")
    print(f"Value: {results['gurobi']['value']}")
    print(f"Time: {results['gurobi']['time']:.3f} seconds")
